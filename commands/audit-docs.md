...
Every finding = stable ID within this report (D1, D2... severity order); a fixing agent cites these. An ID is the prefix plus a plain integer only -- never suffixed, compound, or a severity letter (no D-NEW-1, no D1a, no H1/M1/L1: IDs are ordered by severity, never named by it); new findings just continue the sequence. In the sidecar, severity matches the report's wording (Critical/High/Medium/Low).
