...
- Semantic checks, not just section presence: finding IDs unique and correctly prefixed (C/D/P/S; X for the change audit), an evidence label on every finding, summary table parseable, coverage accounting includes the snapshot SHA (the change audit records it in its Change map section instead).
...
- Map/model facts labeled: the codebase system map, process map, and security threat model carry evidence labels on load-bearing facts (the docs report is exempt -- its verified facts live in its drift-verification section instead).
...
- An audit that failed, timed out, or failed validation is quarantined: excluded from synthesis, listed in the run header. Never discard it silently.
...
- Root-cause dedup: group findings that share one underlying defect. Keep every original ID; preserve evidence-label provenance and state the merge rationale ("one defect: confirmed via C12; D3, P7 corroborating, plausible"). Never collapse confidence levels into one label.
...
1. Run header -- FIRST, always: run directory, snapshot SHA + clean/dirty (for dirty runs, the preflight diff checksum too), audits run / failed / skipped / unavailable with each report's path, model + effort used per audit, drift events. If any selected audit is missing or quarantined, title the document PARTIAL and say what a rerun should cover. Never present a partial suite as complete.
2. Backlog, severity order: each entry = new stable ID (F1, F2...), cited original IDs, merged one-line issue, a one-line behavioral description (what breaks, not where -- line numbers go stale, behavior stays findable), file:line evidence, evidence provenance, acceptance check (from the source findings; Critical/High must have one), suggested fix grouping/order.
3. Conflicts: judgment conflicts + recommendation; fact conflicts + resolution or escalation.
4. Cross-audit patterns: themes multiple audits hit independently -- flag these above their individual severities.
5. What held up: aggregated across reports, one line each.
6. Open questions: merged, deduplicated, maintainer-only.
Alongside the backlog, write <backlog-path>.findings.json (audit: "backlog"; entries carry id F*, severity, the merged one-line issue, cited_ids, behavioral_description, acceptance_check, and evidence_provenance as [{cited_id, evidence}] -- one entry per source finding, never one collapsed label).
...
# Boundaries
You orchestrate and synthesize -- you never fix. The backlog is a plan, not applied changes; auto-chaining into code mutation would erase the authorization boundary the audit prompts establish ("leave uncommitted, maintainer owns git"). All git state belongs to the maintainer.
