...
Every finding = stable ID within this report (P1, P2... severity order); a fixing agent cites these. An ID is the prefix plus a plain integer only -- never suffixed, compound, or a severity letter (no P-NEW-1, no P1a, no H1/M1/L1: IDs are ordered by severity, never named by it); new findings just continue the sequence. In the sidecar, severity matches the report's wording (Critical/High/Medium/Low).
