...
9. Test-suite confidence -- run the suite when safe: a red suite on a clean checkout is itself a finding. Hunt tautological tests that cannot fail, skipped/disabled tests (and how long they have been skipped), and traced critical paths with no covering test. Also hunt implementation-coupled tests (the tell: they break on refactor when behavior didn't change), non-pinning tests (green even when the behavior they claim to pin is broken -- spot-check by reverting or mutating the code under test when cheap), and side-channel verification (asserting via internals or raw storage instead of the public interface). A critical path with no correct seam for a regression test is itself a finding -- name the missing seam (migrated on-disk state and cross-process interleavings are the classic missing seams). When code and docs disagree, use the tests to triangulate which behavior was intended.
...
Every finding = stable ID within this report (C1, C2... severity order); a fixing agent cites these. An ID is the prefix plus a plain integer only -- never suffixed, compound, or a severity letter (no C-NEW-1, no C1a, no H1/M1/L1: IDs are ordered by severity, never named by it); new findings just continue the sequence. In the sidecar, severity matches the report's wording (Critical/High/Medium/Low).
